...
[![Version](https://img.shields.io/badge/Версия-2.15.0-4FC3F7?style=for-the-badge)](https://github.com/qorexdevs/FunPaySigma/releases)
...
- 🌐 Управление прокси
- 📡 Отдельный прокси для Telegram Bot API
...
`/notification_digest` (`/digest`), `/telegram_proxy`.
...
Прокси Telegram отделён от прокси FunPay. Он применяется ко всем запросам
`pytelegrambotapi`, включая long polling, отправку сообщений и встроенные
Telegram-модули. Поддерживаются `http`, `https`, `socks5` и `socks5h`.

- В работающем боте: `/telegram_proxy` или `/menu` → `Инструменты` → `Telegram-прокси`.
- Windows без доступа к боту: запустите `SetupTelegramProxy.bat`.
- Linux без доступа к боту: выполните
  `/home/USER/pyvenv/bin/python /home/USER/FunPaySigma/setup_telegram_proxy.py`,
  затем перезапустите сервис.

Перед сохранением Sigma выполняет настоящий запрос `getMe` через выбранный
прокси. Пароль прокси хранится в конфиге в обфусцированном виде и не показывается
в Telegram-панели.
